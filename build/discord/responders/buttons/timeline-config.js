...
createResponder({
  ...
  async run(interaction) {
    ...
    const timelineChannelName = config?.timelineChannel ? guild.channels.cache.get(config.timelineChannel)?.name : null;
    ...
    const categoryNames = config?.chatCategories?.length ? guild.channels.cache.filter((ch) => config.chatCategories.includes(ch.id)).map((ch) => ch.name).join(", ") : null;
    const categorySelect = new ChannelSelectMenuBuilder({
      customId: "timeline-config/select/category",
      channelTypes: [4],
      placeholder: categoryNames ? categoryNames.slice(0, 50) + (categoryNames.length > 50 ? "..." : "") : "Selecione as categorias de chat",
      minValues: 0,
      maxValues: 25
    });
