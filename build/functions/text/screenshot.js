...
async function generateTweetHTML(params) {
  const {
    currentUserName,
    currentMessage,
...
.verified-badge {
  ...
  `;
  const privateIcon = isPrivateQuote ? '<span class="private">\xB0</span>' : "";
  const currentImageHTML = currentImageUrl ? `<img class="tweet-image" src="${currentImageUrl}" alt="Imagem da mensagem atual" />` : "";
  const previousImageHTML = previousImageUrl ? `<img class="tweet-image" src="${previousImageUrl}" alt="Imagem da mensagem citada" />` : "";
  return `
    <html>
      <head>
        <style>${css}</style>
      ...
      <body>
        <div class="tweet-container">
          ...
          <div class="tweet-footer">
            <span>${(/* @__PURE__ */ new Date()).toLocaleTimeString()} \xB7 ${(/* @__PURE__ */ new Date()).toLocaleDateString()}</span>
